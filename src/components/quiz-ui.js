// File Path: src/components/quiz-ui.js
        ...
        if (q.type === 'ai_prompt') {
            ...
            return `
                <div class="quiz-island" data-question-index="${index}" data-grid-area="6" style="transition-delay: ${index * 100}ms;">
                    <h3 class="text-xl font-bold text-center mb-2">${q.question}</h3>
                    <p class="text-gray-600 text-center text-sm mb-4">${q.description}</p>
            ...
            const optionsHTML = q.options.map(opt => `
                <button class="quiz-option w-full text-left p-4 flex items-center gap-4" data-tags="${opt.tags.join(',')}" data-question-index="${index}">
                    ...
                    <span class="font-semibold text-base md:text-lg">${opt.text}</span>
                </button>`).join('');

            return `
                <div class="quiz-island" data-question-index="${index}" data-grid-area="${gridAreas[index] || ''}" style="transition-delay: ${index * 100}ms;">
                    <h3 class="text-xl font-bold text-center mb-4">${q.question}</h3>
    ...
    quizContent.innerHTML = `
        <div class="quiz-result-view text-center max-w-3xl mx-auto py-8">
            ...
            <div id="quiz-result-card" class="my-8 p-6 bg-white/80 rounded-2xl border text-left">
                <div class="flex flex-col sm:flex-row items-center gap-6 mb-6">
                    ...
                    <div class="text-center sm:text-left">
                        <h4 class="text-2xl font-bold">${bestMatch.name}</h4>
                        <p class="text-gray-600 mt-1 italic">"${bestMatch.description}"</p>
                    </div>
                </div>
            ...
            <div class="flex flex-wrap gap-4 justify-center">
                ...
                <button id="downloadQuizResultPngBtn" data-element-id="quiz-result-card" data-recipe-name="${bestMatch.name}" class="btn bg-gray-700 hover:bg-gray-800 text-white py-3 px-6 shadow-lg shadow-gray-500/30">
            ...
            <form id="aiRecipeForm">
                <div id="quiz-ai-result-card" class="my-8 p-6 bg-white/80 rounded-2xl border text-left space-y-4">
                    <div>
                        ...
                        <input type="text" id="recipeName" class="w-full text-2xl font-bold text-center p-2 border-b-2" value="${recipe.name}">
                    </div>
                    <div>
                         <label for="recipeDescription" class="text-base font-bold mb-2 block" data-translate-key="recipeDescriptionLabel"></label>
                        <textarea id="recipeDescription" rows="2" class="w-full text-gray-600 mt-1 text-center italic p-2 border-b-2">${recipe.description}</textarea>
                    </div>

  ...
  container.innerHTML = recipesToRender.map((recipe, index) => {
    ...
    return `<div class="recipe-item p-4 rounded-lg cursor-pointer ${isSelected ? 'selected' : ''} recipe-item-stagger"
            ...
            <div class="flex justify-between items-start">
                <span class="font-semibold text-primary pr-2">${recipe.formattedName}</span>
...
function createRecipeDetailHTML(originalRecipe) {
  ...
  const createCollageHTML = (recipe) => {
    ...
    const imageElements = images.map((imgUrl, index) => {
      return `
      <div class="collage-item" data-recipe-id="${recipe.id}" data-index="${index}" key="${index}">
          <img 
              src="${imgUrl}" 
              loading="lazy" 
              decoding="async"
              alt="Demo image ${index + 1} for ${recipe.name}"
  ...
  return `
        ...
        <div class="mt-6 flex flex-col sm:flex-row justify-between items-start gap-4">
            <div>
                <h3 class="text-3xl md:text-4xl font-bold">${recipe.formattedName}</h3>
...
export function renderLibraryDetails() {
  ...
  const recipeContentContainer = isMobile && state.ui.isMobileDetailActive
  ...
  if (recipe) {
    // A recipe is selected, so hide the welcome guide and show the recipe details.
    ...
    // No recipe is selected, so show the welcome guide and hide the recipe details.
    if (welcomeContainer) welcomeContainer.classList.remove('hidden');
    recipeContentContainer.classList.add('hidden');
    recipeContentContainer.innerHTML = ''; // Clear the content
  }

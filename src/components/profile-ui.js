...
export async function renderUserProfilePage(userId) {
    ...
    const recentlySelectedRecipes = state.allRecipes.filter(r => favoriteRecipeIds.slice(0, 4).includes(r.id));
...
function createDemoPhotoSubmitModal() {
    const recipeOptions = state.allRecipes.map(r => `<option value="${r.id}">${formatRecipeName(r.name[state.language])}</option>`).join('');

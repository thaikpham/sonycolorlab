...
import { forceSimulation, forceCollide, forceX, forceY, forceCenter } from 'd3-force';
...
export async function renderColorMapChart(containerSelector, data) {
  ...
  const nodesData = data.filter(d =>
    d.coords &&
    typeof d.coords.x === 'number' && isFinite(d.coords.x) &&
    typeof d.coords.y === 'number' && isFinite(d.coords.y)
  ).map(d => ({ ...d, isTrending: trendingIds.includes(d.id) }));

  // Initialize node positions to prevent simulation explosion
  nodesData.forEach(d => {
    d.x = xScale(d.coords.x);
    d.y = yScale(d.coords.y);
  });
  ...
  state.chart.nodes.append("circle")
    ...
    .attr("r", 12)
    .attr("fill", d => d.personalityColor || 'slateGray')
  ...
  state.chart.nodes.attr("transform", d => `translate(${d.coords.x}, ${d.coords.y})`);

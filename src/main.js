...
async function fetchTrendingRecipes() {
    ...
    try {
        const docRef = doc(state.firebase.db, "artifacts/default-app-id/public/data/trending/latest");
...
function renderView(viewName, selectedId = null) {
    ...
    if (viewName !== 'home') {
        document.body.style.overflowY = 'auto'; // Cho phép scroll lại ở các view khác
        blobContainer.querySelectorAll('.bg-blob').forEach(b => b.classList.remove('visible'));
        setTimeout(() => { blobContainer.innerHTML = ''; }, 1500);
    } else {
        document.body.style.overflowY = 'hidden'; // Khóa scroll ở trang chủ
    }
...
/**
 * Nâng cấp: Khởi tạo các chấm màu với hiệu ứng vật lý D3.js
 * Tạo ra các chấm màu di chuyển, va chạm và tương tác với nhau.
 */
function initializePhysicsBlobs() {
    // Sửa lỗi: Kiểm tra xem D3 đã tải xong chưa trước khi chạy
    if (typeof d3 === 'undefined') {
        console.warn("D3 is not loaded yet. Retrying in 100ms.");
        setTimeout(initializePhysicsBlobs, 100); // Thử lại sau 100ms
        return;
    }
    ...
    container.html(''); // Xóa các blob cũ nếu có

    const vw = window.innerWidth;
    const vh = window.innerHeight;

    // Dữ liệu cho các chấm màu (nodes) với nhiều kích thước hơn
    const nodes = [
        { id: 1, color: '#ffae00', r: vw * 0.12 },
        { id: 2, color: '#3498db', r: vw * 0.15 },
        { id: 3, color: '#e74c3c', r: vw * 0.11 },
        { id: 4, color: '#2ecc71', r: vw * 0.14 },
        { id: 5, color: '#9b59b6', r: vw * 0.10 },
        { id: 6, color: '#1abc9c', r: vw * 0.13 },
        { id: 7, color: '#f1c40f', r: vw * 0.09 },
        { id: 8, color: '#e67e22', r: vw * 0.12 },
        { id: 9, color: '#34495e', r: vw * 0.10 },
        { id: 10, color: '#d35400', r: vw * 0.11 },
    ].map(d => ({
        ...d,
        x: Math.random() * vw,
        y: Math.random() * vh,
        vx: (Math.random() - 0.5) * 4, // Vận tốc ban đầu
        vy: (Math.random() - 0.5) * 4,
    }));

    // Tạo các element div cho mỗi chấm màu
    ...
    setTimeout(() => {
        blobElements.classed("visible", true);
    }, 100);

    // Thiết lập mô phỏng vật lý
    const simulation = d3.forceSimulation(nodes)
        .alphaTarget(0.3) // Giữ cho simulation luôn "nóng"
        .velocityDecay(0.1) // Giảm ma sát để chúng di chuyển lâu hơn
        .force("collide", d3.forceCollide().radius(d => d.r + 8).strength(0.8)) // Tăng khoảng cách va chạm, giảm nhẹ lực
        .force("charge", d3.forceManyBody().strength(-50)) // Tăng lực đẩy
        .on("tick", ticked);

    // Hàm được gọi mỗi khi có sự thay đổi vị trí trong mô phỏng
    function ticked() {
        blobElements.each(d => {
            // Giữ các chấm màu trong màn hình với hiệu ứng nảy nhẹ
    ...
    // Thêm một lực "gió" nhẹ và ngẫu nhiên liên tục
    d3.timer(() => {
        nodes.forEach(node => {
            node.vx += (Math.random() - 0.5) * 0.3;
            node.vy += (Math.random() - 0.5) * 0.3;
        });
        simulation.alpha(0.1).restart(); // "Hâm nóng" lại mô phỏng
    });
}
...
function initializeInteractiveLogo() {
    if (typeof d3 === 'undefined') {
        setTimeout(initializeInteractiveLogo, 100);
        return;
    }
    ...
    const nodes = [
        ...
        { id: 'cyan', color: '#1abc9c' },
        { id: 'magenta', color: '#9b59b6' },
        { id: 'yellow', color: '#f1c40f' },
        { id: 'grey', color: '#95a5a6' },
...
function attachViewEventListeners(viewName) {
    if (viewName === 'home') {
        // Thay thế animation cũ bằng animation vật lý mới
...
async function initializeFirebase() {
    if (typeof __firebase_config === 'undefined' || typeof __app_id === 'undefined' || __firebase_config === "%%FIREBASE_CONFIG%%" || __app_id === "%%APP_ID%%") {
